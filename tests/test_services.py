...
class TestAtomization:
    """Tests for atomization helpers."""

    def test_select_atoms_for_linkedin(self, sample_atoms):
        """Test selecting atoms for LinkedIn."""
        selected = select_atoms_for_content_type(
            sample_atoms, "linkedin", "ceo_longterm_care", count=2
        )
        assert len(selected) <= 2
        # Data atoms should be preferred for LinkedIn
        types = [a["atom_type"] for a in selected]
        assert "data" in types or "insight" in types

    def test_select_atoms_for_blog(self, sample_atoms):
        """Test selecting atoms for blog."""
        selected = select_atoms_for_content_type(
            sample_atoms, "blog", "ceo_longterm_care", count=3
        )
        assert len(selected) <= 3

    def test_group_atoms_by_type(self, sample_atoms):
        """Test grouping atoms by type."""
        grouped = group_atoms_by_type(sample_atoms)
    ...
    def test_empty_atoms(self):
        """Test with empty atoms list."""
        selected = select_atoms_for_content_type([], "linkedin", "ceo_longterm_care")
        assert len(selected) == 0

        grouped = group_atoms_by_type([])
...
class TestCostCalculation:
    ...
    def test_calculate_cost_gemini_flash(self):
        ...
        cost = calculate_cost("gemini-2.0-flash", 1000000, 500000)
        expected = (1000000 / 1000000) * 0.075 + (500000 / 1000000) * 0.30
        assert cost == pytest.approx(expected)

    def test_calculate_cost_claude_opus(self):
        """Test cost calculation for Claude Opus."""
        cost = calculate_cost("claude-opus-4-5-20251101", 10000, 5000)
        expected = (10000 / 1000000) * 15.00 + (5000 / 1000000) * 75.00
        assert cost == pytest.approx(expected)
